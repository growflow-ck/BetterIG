...
(function createBetterIGBackend() {
  ...
  window.BetterIGBackend = Object.freeze({
    ...
    async signUpWithPassword(email, password) {
      ...
      if (error) throw error;
      if (!data.session) throw new Error("Turn off Confirm email in Supabase before creating accounts.");
      return data.session;
    },
    ...
    resolveRoulette(bet, stakeRatio) {
      return rpc("resolve_roulette", { p_bet: bet, p_stake_ratio: stakeRatio });
    },
    resolveMarket(prediction, stakeRatio) {
      return rpc("resolve_market", { p_prediction: prediction, p_stake_ratio: stakeRatio });
    },
    resolvePrizeWheel() { return rpc("resolve_prize_wheel"); },

    async getLeaderboard() {
      const { data, error } = await requireClient()
        .from("profiles")
        .select("user_id,username,avatar_url,score,total_lost")
