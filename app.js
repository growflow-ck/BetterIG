...
const els = {
  ...
  authStep: document.querySelector("#authStep"), authModeSwitch: document.querySelector("#authModeSwitch"), emailAuthForm: document.querySelector("#emailAuthForm"), emailInput: document.querySelector("#emailInput"), passwordInput: document.querySelector("#passwordInput"), passwordToggle: document.querySelector("#passwordToggle"), emailSignIn: document.querySelector("#emailSignIn"), authError: document.querySelector("#authError"), privacyNote: document.querySelector("#privacyNote"),
...
function applyRemoteState(remote) {
  ...
  state.user.nextRouletteAt = Number(remote.next_roulette_at) || state.user.nextRouletteAt;
  state.user.nextMarketAt = Number(remote.next_market_at) || state.user.nextMarketAt;
...
function renderLeaderboard() {
  const ranked = state.leaders.map(item => ({
    ...
    score: item.user_id === state.remoteUserId ? state.user.score : (Number(item.score) || 0),
    avatar: item.avatar_url || "",
    current: item.user_id === state.remoteUserId
  }));
  let current = ranked.find(item => item.current);
  if (!current && state.user.username) {
    current = { userId: state.remoteUserId, username: state.user.username, score: state.user.score, avatar: "", current: true };
    ranked.push(current);
...
async function spinRoulette() {
  ...
  let result;
  try {
    result = await backend.resolveRoulette(state.selectedBet);
  } catch (error) {
    state.rouletteSpinning = false;
    setSelectedBet(state.selectedBet);
    els.rouletteSpin.disabled = false;
    els.rouletteSpin.textContent = `Bet everything on ${state.selectedBet}`;
...
function setAuthMode(mode) {
  ...
  els.authModeSwitch.querySelectorAll("button").forEach(button => {
    button.setAttribute("aria-pressed", String(button.dataset.authMode === state.authMode));
  });
  els.passwordInput.autocomplete = state.authMode === "signup" ? "new-password" : "current-password";
  els.emailSignIn.innerHTML = state.authMode === "signup"
...
async function finishAuthentication(session) {
  ...
  const suggested = String(session.user.email?.split("@")[0] || "")
    .replace(/[^A-Za-z0-9_]/g, "_")
    .slice(0, 18);
  els.usernameInput.value = suggested.length >= 2 ? suggested : "";
  els.authStep.hidden = true;
  els.onboardingForm.hidden = false;
  els.privacyNote.textContent = "Account created. Choose the unique name your friends will see.";
...
els.emailAuthForm.addEventListener("submit", async event => {
  ...
  if (!email || !els.emailInput.validity.valid) {
    els.authError.textContent = "Enter a valid email address.";
    els.emailInput.focus();
    return;
  }
  if (password.length < 8) {
    els.authError.textContent = "Use a password with at least 8 characters.";
    els.passwordInput.focus();
    return;
  }
  els.emailSignIn.disabled = true;
  els.emailSignIn.textContent = state.authMode === "signup" ? "Creating account…" : "Logging in…";
  els.authError.textContent = "";
  try {
    const session = state.authMode === "signup"
      ? await backend.signUpWithPassword(email, password)
      : await backend.signInWithPassword(email, password);
    await finishAuthentication(session);
  } catch (error) {
    const message = error.message || "Couldn’t access that account.";
    let friendlyMessage;
    if (/already registered/i.test(message)) {
      friendlyMessage = "An account already uses this email. Choose Log in.";
    } else if (/invalid login credentials/i.test(message)) {
      friendlyMessage = "Wrong email or password.";
    } else {
      friendlyMessage = message;
    }
    els.emailSignIn.disabled = false;
    setAuthMode(state.authMode);
    els.authError.textContent = friendlyMessage;
  }
});
els.emailInput.addEventListener("input", () => { els.authError.textContent = ""; });
els.passwordInput.addEventListener("input", () => { els.authError.textContent = ""; });
els.signOutButton.addEventListener("click", async () => {
  els.signOutButton.disabled = true;
  els.signOutButton.textContent = "Signing out…";
  try {
    await backend.signOut();
    location.reload();
  } catch {
    els.signOutButton.disabled = false;
    els.signOutButton.textContent = "Sign out";
...
async function bootstrap() {
  ...
  if (!backend?.available) {
    els.authError.textContent = "BetterIG couldn’t load its online connection. Refresh and try again.";
    els.emailSignIn.disabled = true;
    return;
  }
  try {
    ...
    await finishAuthentication(state.authSession);
  } catch (error) {
    els.authError.textContent = error.message?.includes("function")
      ? "The BetterIG database setup still needs to be installed."
