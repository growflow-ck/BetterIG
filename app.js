...
const els = {
  ...
  authStep: document.querySelector("#authStep"), githubSignIn: document.querySelector("#githubSignIn"), authError: document.querySelector("#authError"), privacyNote: document.querySelector("#privacyNote"),
...
els.githubSignIn.addEventListener("click", async () => {
  els.githubSignIn.disabled = true;
  els.githubSignIn.textContent = "Opening GitHub…";
  els.authError.textContent = "";
  try {
    await backend.signInWithGitHub();
  } catch (error) {
    els.authError.textContent = error.message || "Couldn’t start GitHub sign-in.";
    els.githubSignIn.disabled = false;
    els.githubSignIn.innerHTML = `Continue with GitHub <span aria-hidden="true">→</span>`;
  }
});
...
async function bootstrap() {
  ...
  try {
    ...
    if (!state.authSession) {
      setTimeout(() => els.githubSignIn.focus(), 100);
      return;
    }

    state.remoteUserId = state.authSession.user.id;
    ...
    const suggested = String(
      state.authSession.user.user_metadata?.preferred_username ||
      state.authSession.user.user_metadata?.user_name || ""
    ).replace(/[^A-Za-z0-9_]/g, "_").slice(0, 18);
    els.usernameInput.value = suggested.length >= 2 ? suggested : "";
    els.authStep.hidden = true;
    els.onboardingForm.hidden = false;
    els.privacyNote.textContent = "Signed in. Choose the name your friends will see on the leaderboard.";
