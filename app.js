 ...
 * ----------------------------
 * The mock data and storage adapter are isolated below so localStorage can later
 * be swapped for real authentication, reels, and leaderboard API services.
 */
...
const MOCK_LEADERS = [
  { username: "scrollsroyce", score: 3842, avatar: "" },
  { username: "thumbelina", score: 3617, avatar: "" },
  { username: "reeldeal", score: 3254, avatar: "" },
  { username: "loopmaster", score: 2889, avatar: "" },
  { username: "pixelpiper", score: 2156, avatar: "" },
  { username: "nightowl.mov", score: 1644, avatar: "" },
  { username: "sunday.scroller", score: 904, avatar: "" },
  { username: "tinyturns", score: 211, avatar: "" }
];

const STORAGE_KEY = "betterig.user.v1";
const storage = {
  load() {
    try {
...
const ROULETTE_SCHEDULE_VERSION = 2;
function randomRouletteGap() { return Math.floor(Math.random() * 21) + 40; }
...
const user = {
  ...
  roulette: null,
  rouletteScheduleVersion: ROULETTE_SCHEDULE_VERSION,
  marketPending: false,
  market: null,
  nextMarketAt: 0,
  muted: true,
...
user.nextRouletteAt = Number.isFinite(user.nextRouletteAt) && user.nextRouletteAt > user.totalScrolls
  ? user.nextRouletteAt : user.totalScrolls + randomRouletteGap();
...
if (savedUser?.rouletteScheduleVersion !== ROULETTE_SCHEDULE_VERSION && !user.roulettePending) {
  user.nextRouletteAt = user.totalScrolls + randomRouletteGap();
}
user.rouletteScheduleVersion = ROULETTE_SCHEDULE_VERSION;
user.nextMarketAt = Number.isFinite(user.nextMarketAt) && user.nextMarketAt > user.totalScrolls
  ? user.nextMarketAt : (Math.floor(user.totalScrolls / 100) + 1) * 100;
storage.save(user);
...
const els = {
  ...
  leaderboardPanel: document.querySelector("#leaderboardPanel"), leaderboardList: document.querySelector("#leaderboardList"), leaderboardEmpty: document.querySelector("#leaderboardEmpty"),
  ...
  betOptions: document.querySelector("#betOptions"), rouletteSpin: document.querySelector("#rouletteSpin"), rouletteResult: document.querySelector("#rouletteResult"), rouletteContinue: document.querySelector("#rouletteContinue"),
...
function gatePending() { return state.user.roulettePending || state.user.marketPending; }
...
function createReel(reel, index) {
  ...
  article.innerHTML = `
    ...
    <video class="meme-media" src="${reel.src}" playsinline loop preload="metadata" aria-label="Meme video ${index + 1}"></video>
...
function activateReel(reelEl) {
  ...
  if (state.initialized && previousInstance !== null) awardPoint();
...
function awardPoint() {
  state.user.score += 1;
  state.user.totalScrolls += 1;
  const reachedMilestone = state.user.totalScrolls % 10 === 0;
  if (state.user.totalScrolls >= state.user.nextMarketAt) {
    state.user.marketPending = true;
    state.user.market = null;
  }
  if (!state.user.marketPending && state.user.totalScrolls >= state.user.nextRouletteAt) {
    state.user.roulettePending = true;
    state.user.roulette = null;
  } else if (state.user.marketPending && state.user.totalScrolls >= state.user.nextRouletteAt) {
    state.user.nextRouletteAt = state.user.totalScrolls + randomRouletteGap();
  }
  ...
  if (state.user.marketPending) setTimeout(openMarket, gateDelay);
...
function toggleLike(reelEl, forceLike = false) {
  ...
  document.querySelectorAll(`[data-reel-id="${reelId}"] .like-button`).forEach(button => {
    ...
    button.setAttribute("aria-label", `${nextLiked ? "Unlike" : "Like"} this meme`);
    button.querySelector(".action-icon").textContent = nextLiked ? "♥" : "♡";
    button.querySelector(".action-count").textContent = formatCount(reel.likes + (nextLiked ? 1 : 0));
  });
}
...
function renderLeaderboard() {
  const current = { username: state.user.username || "you", score: state.user.score, avatar: "" , current: true };
  const ranked = [...MOCK_LEADERS, current].sort((a, b) => b.score - a.score || a.username.localeCompare(b.username));
  const currentRank = ranked.findIndex(user => user.current) + 1;
  const visible = ranked.slice(0, 8);
  if (!visible.some(user => user.current) && state.user.score > 0) visible.push(current);
  els.leaderboardList.innerHTML = visible.map(user => leaderRow(user, ranked.findIndex(item => item === user) + 1)).join("");
  els.currentUserDock.innerHTML = leaderRow(current, currentRank, true);
...
function leaderRow(user, rank, dock = false) {
  ...
  const label = user.current ? "YOU" : rank <= 3 ? ["SCROLL CHAMPION", "RUNNER UP", "TOP THREE"][rank - 1] : dock ? "YOUR POSITION" : "";
  return `<div class="leader-row ${user.current ? "is-current" : ""} ${rank <= 3 ? `top-${rank}` : ""}">
    ...
    <span class="leader-score">${user.score.toLocaleString()}</span>
  </div>`;
}
...
function resolveMarket() {
  ...
  const balanceBefore = state.user.score;
  const stake = Math.max(1, Math.floor(balanceBefore * state.selectedStakeRatio));
  const outcome = Math.random() < .5 ? "up" : "down";
  const win = outcome === state.selectedPrediction;
  const winnings = win ? stake * 2 : 0;
  const finalScore = balanceBefore - stake + winnings;
  const round = { prediction: state.selectedPrediction, stakeRatio: state.selectedStakeRatio, stake, outcome, win, winnings, finalScore };

  // Save the resolved market before revealing it so refreshes cannot reroll it.
  state.user.market = round;
  state.user.score = finalScore;
  storage.save(state.user);
  updateScore();
  renderLeaderboard();
...
function completeMarket() {
  ...
  state.user.market = null;
  state.user.nextMarketAt = (Math.floor(state.user.totalScrolls / 100) + 1) * 100;
  storage.save(state.user);
  state.selectedPrediction = null;
  state.selectedStakeRatio = null;
...
function openRoulette() {
  ...
  els.rouletteWager.textContent = state.user.roulette?.wager?.toLocaleString() || state.user.score.toLocaleString();
  ...
  if (state.user.roulette) {
    ...
    els.rouletteSpin.textContent = "Choose a color";
...
function selectBet(color) {
  ...
  els.rouletteSpin.textContent = `Bet everything on ${color}`;
}

function spinRoulette() {
  if (!state.selectedBet || state.rouletteSpinning || state.user.roulette) return;
  state.rouletteSpinning = true;
  setSelectedBet(state.selectedBet);
  els.rouletteSpin.disabled = true;
  els.rouletteSpin.textContent = "Spinning…";

  const outcomeIndex = Math.floor(Math.random() * ROULETTE_SEGMENTS.length);
  const outcome = ROULETTE_SEGMENTS[outcomeIndex];
  const wager = state.user.score;
  const multiplier = state.selectedBet === "green" ? 7 : 2;
  const win = outcome === state.selectedBet;
  const payout = win ? wager * multiplier : 0;
  const round = { bet: state.selectedBet, wager, outcome, outcomeIndex, win, payout };

  // Resolve and save before the animation so refreshing cannot reroll the result.
  state.user.roulette = round;
  state.user.score = payout;
  storage.save(state.user);
  updateScore();
  renderLeaderboard();
  ...
  els.rouletteWheel.style.transform = `rotate(${2160 - outcomeIndex * SEGMENT_ANGLE}deg)`;
...
function showRouletteResult(round) {
  ...
  els.rouletteResult.textContent = round.win
    ...
    : `${round.outcome.toUpperCase()}. You lost all ${round.wager.toLocaleString()} scrolls.`;
...
function trapModalFocus(event) {
  ...
  const modal = els.marketGate.classList.contains("is-open") ? els.marketGate
...
function enterApp() {
  ...
  els.app.setAttribute("aria-hidden", "false");
  updateScore(); renderLeaderboard();
  if (!els.feed.children.length) appendReelBatch();
  if (state.user.marketPending) setTimeout(openMarket, 100);
...
document.addEventListener("keydown", event => {
  ...
  if (els.marketGate.classList.contains("is-open") || els.rouletteGate.classList.contains("is-open")) {
...
if (state.user.username) enterApp();
else {
  els.app.setAttribute("aria-hidden", "true");
